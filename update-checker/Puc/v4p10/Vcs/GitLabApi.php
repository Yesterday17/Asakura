	...
	class Puc_v4p10_Vcs_GitLabApi extends Puc_v4p10_Vcs_Api {
		...
		public function getLatestTag() {
			...
			return new Puc_v4p10_Vcs_Reference(array(
				'name'        => $tag->name, 'version' => ltrim($tag->name, 'v'),
				'downloadUrl' => $this->buildArchiveDownloadUrl($tag->name), 'apiResponse' => $tag,
			));
		}
		...
		public function getBranch($branchName) {
			...
			$reference = new Puc_v4p10_Vcs_Reference(array(
				'name'        => $branch->name, 'downloadUrl' => $this->buildArchiveDownloadUrl($branch->name),
				'apiResponse' => $branch,
			));
		...
		protected function buildApiUrl($url, $queryParams) {
			$variables = array(
				'user' => $this->userName, 'repo' => $this->repositoryName,
