...
if (!class_exists('Puc_v4p10_Vcs_BitBucketApi', false)):
	...
	class Puc_v4p10_Vcs_BitBucketApi extends Puc_v4p10_Vcs_Api {
		...
		public function api($url, $version = '2.0') {
			...
			$url = implode('/', array(
				'https://api.bitbucket.org', $version, 'repositories', $this->username, $this->repository, $url
			));
			$baseUrl = $url;
		...
		public function setAuthentication($credentials) {
			...
			if (!empty($credentials) && !empty($credentials['consumer_key'])) {
				$this->oauth = new Puc_v4p10_OAuthSignature($credentials['consumer_key'], $credentials['consumer_secret']);
			} else {
				$this->oauth = null;
			}
		}
