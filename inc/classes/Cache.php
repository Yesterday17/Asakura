...
EOS;
        ...
        while ($posts->have_posts()): $posts->the_post();
            $output[] = array(
                "type"     => "post", "link" => get_permalink(), "title" => get_the_title(),
        ...
        while ($pages->have_posts()): $pages->the_post();
            $output[] = array(
                "type"     => "page", "link" => get_permalink(), "title" => get_the_title(),
                "comments" => get_comments_number('0', '1', '%'),
                "text"     => str_replace($vowels, " ", preg_replace($regex, ' ', apply_filters('the_content', get_the_content())))
            );
        endwhile;
        ...
        foreach ($tags as $tag) {
            $output[] = array(
                "type" => "tag", "link" => get_term_link($tag), "title" => $tag->name, "comments" => "", "text" => ""
            );
        }
        ...
        foreach ($categories as $category) {
            $output[] = array(
                "type" => "category", "link" => get_term_link($category), "title" => $category->name, "comments" => "",
                "text" => ""
            );
        }
        if (akina_option('live_search_comment')) {
            ...
            foreach ($comments as $comment) {
                ...
                $output[] = array(
                    "type"  => "comment", "link" => get_comment_link($comment),
                    "title" => get_the_title($comment->comment_post_ID), "comments" => "",
