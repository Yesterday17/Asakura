...
function get_random_bg_url() {
    return rest_url('/v1/image/feature') . '?' . rand(1, 1000);
}
...
function poi_time_since($older_date, $comment_date = false, $text = false) {
    $chunks = array(
        array(24 * 60 * 60, __(' days ago', SAKURA_DOMAIN)),/*天前*/
        array(60 * 60, __(' hours ago', SAKURA_DOMAIN)),/*小时前*/
        array(60, __(' minutes ago', SAKURA_DOMAIN)),/*分钟前*/
        array(1, __(' seconds ago', SAKURA_DOMAIN))/*秒前*/
    );
    ...
    if ($text) {
        ...
        $output = __('Posted on ', SAKURA_DOMAIN)/*发布于*/
        ;
    }
...
function comment_add_at($comment_text, $comment = '') {
    if ($comment->comment_parent > 0) {
        if (substr($comment_text, 0, 3) === "<p>")
            $comment_text = str_replace(substr($comment_text, 0, 3), '<p><a href="#comment-' . $comment->comment_parent . '" class="comment-at">@' . get_comment_author($comment->comment_parent) . '</a>&nbsp;', $comment_text); else
...
if (version_compare($GLOBALS['wp_version'], '4.4-alpha', '<')) {
    wp_die(__('Please upgrade wordpress to version 4.4+', SAKURA_DOMAIN));
}/*请升级到4.4以上版本*/
// 提示
                            ...
                            <section class="commeta">
                                <div class="left">
                                    <h4 class="author"><a
                                            ...
                                            <span class="isauthor"
                                                  title="<?php esc_attr_e('Author', SAKURA_DOMAIN); ?>"></span></a></h4>
                                </div>
                                <div class="right">
                                    <div class="info">
...
function Exuser_center() { ?>
    <script language='javascript' type='text/javascript'>
        ...
        function gopage(url, type) {
            URL = url;
            if (type == 1) {
                TYPE = <?php _e('dashboard', SAKURA_DOMAIN)/*管理后台*/?>;
            } else {
                TYPE = <?php _e('home', SAKURA_DOMAIN)/*主页*/?>;
            }
        ...
        function doUpdate(num) {
            document.getElementById('login-showtime').innerHTML = '<?php _e("Login successful, ", SAKURA_DOMAIN)/*空降成功*/?>' + num + '<?php _e("seconds later automatically transfer to", SAKURA_DOMAIN)/*秒后自动转到*/?>' + TYPE;
            if (num == 0) {
                window.location = URL;
            }
        }
...
function login_ok() {
    ...
    wp_get_current_user();
    ?>
    <p class="ex-login-avatar"><a href="http://cn.gravatar.com/"
                                  title="<?php _e('Change avatar', SAKURA_DOMAIN)/*更换头像*/ ?>" target="_blank"
                                  rel="nofollow"><?php echo get_avatar($current_user->user_email, '110'); ?></a></p>
    <p class="ex-login-username"><?php _e('Hello, ', SAKURA_DOMAIN)/*你好，*/ ?>
    ...
    <p class="ex-logout">
        <a href="<?php bloginfo('url'); ?>"
           title="<?php _e('Home', SAKURA_DOMAIN)/*首页*/ ?>"><?php _e('Home', SAKURA_DOMAIN)/*首页*/ ?></a>
        <?php if (current_user_can('level_10')) { ?>
            <a href="<?php bloginfo('url'); ?>/wp-admin/" title="<?php _e('Manage', SAKURA_DOMAIN)/*后台*/ ?>"
               target="_top"><?php _e('Manage', SAKURA_DOMAIN)/*后台*/ ?></a>
        <?php } ?>
        <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" title="<?php _e('Logout', SAKURA_DOMAIN)/*登出*/ ?>"
           target="_top"><?php _e('Sign out? ', SAKURA_DOMAIN)/*登出？*/ ?></a>
    </p>
    <?php
}
...
function the_headPattern() {
    ...
    if (is_single()) {
        ...
        $t .= '<h1 class="entry-title search-title"> ' . sprintf(__("Search results for \" %s \"", SAKURA_DOMAIN), get_search_query()) ./*关于“ '.get_search_query().' ”的搜索结果*/
            '</h1>';
    }
    ...
    if (!is_home() && $full_image_url) : ?>
        ...
        <div class="pattern-center <?php if (is_single()) {
            ...
            <div class="pattern-attachment-img lazyload"
                 style="background-image: url(<?php echo akina_option('webweb_img'); ?>/load/outload.svg)"
...
function the_video_headPattern_normal() {
    ...
    if (is_single()) {
        ...
        if (have_posts()) : while (have_posts()) : the_post();
            ...
            global $user_ID;
            if ($user_ID && current_user_can('level_10')) {
                $edit_this_post_link = '<span class="bull">·</span><a href="' . get_edit_post_link() . '">' . _e("EDIT", SAKURA_DOMAIN) . '</a>';
            } else {
                $edit_this_post_link = '';
            }
            $t .= the_title('<h1 class="entry-title">', '<button id="coverVideo-btn" class=".constant-width-to-height-ratio" onclick="coverVideo()"><i class="fa fa-pause" aria-hidden="true"></i></button></h1>', false);
                ...
                <div class="user-menu-option">
                    <?php if (current_user_can('level_10')) { ?>
                        <a href="<?php bloginfo('url'); ?>/wp-admin/"
                           target="_blank"><?php _e('Dashboard', SAKURA_DOMAIN)/*管理中心*/ ?></a>
                        <a href="<?php bloginfo('url'); ?>/wp-admin/post-new.php"
                           target="_blank"><?php _e('New post', SAKURA_DOMAIN)/*撰写文章*/ ?></a>
                    <?php } ?>
                    <a href="<?php bloginfo('url'); ?>/wp-admin/profile.php"
                       target="_blank"><?php _e('Profile', SAKURA_DOMAIN)/*个人资料*/ ?></a>
                    <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>"
                       target="_top"><?php _e('Sign out', SAKURA_DOMAIN)/*退出登录*/ ?></a>
                </div>
            </div>
        </div>
        <?php
...
function no_category_base_rewrite_rules($category_rewrite) {
    ...
    foreach ($categories as $category) {
        ...
        if ($category->parent == $category->cat_ID)// recursive recursion
            $category->parent = 0; elseif ($category->parent != 0)
...
function siren_private_message_hook($comment_content, $comment) {
    ...
    if ($is_private)
        return '<i class="fa fa-lock" aria-hidden="true"></i> ' . __("The comment is private", SAKURA_DOMAIN)/*该评论为私密评论*/ ;
    return $comment_content;
}
...
function siren_get_browsers($ua) {
    ...
    if (preg_match('#MSIE ([a-zA-Z0-9.]+)#i', $ua, $matches)) {
        ...
        if (strpos($matches[1], '7') !== false || strpos($matches[1], '8') !== false)
            $icon = 'ie8'; elseif (strpos($matches[1], '9') !== false)
            $icon = 'ie9';
...
function siren_get_os($ua) {
    ...
    if (preg_match('/win/i', $ua)) {
        ...
        if (count(explode(7, $matches[1])) > 1)
            $matches[1] = 'Lion ' . $matches[1]; elseif (count(explode(8, $matches[1])) > 1)
